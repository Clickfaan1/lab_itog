...
const API_KEY = 'c329af30-f26a-4176-b19f-bebdf3aa1921'; 
...
function addRoutesToTable(routes) {
    ...
    routes.forEach(route => {
        ...
        selectButton.classList = 'btn btn-primary';
        selectButton.addEventListener('click', () => guideDownload(route));
        row.insertCell(3).appendChild(selectButton);
    });
}

//рассчитывается, является ли день выходным и 
//возвращаем кэффициент умножения стоимости
...
//Тут мы берем значения из инпутов и вызываем функцию с рассчетом цены
...
//тут отправляются методом PUT заявка в ЛК
...
//берем уникальные значения из массива q языков
function getoptionforselect(q) {
    return [... new Set(q)];    
}

//чистка селекта фильтра для гидов от языков
...
//генерация селекта с массивом языков
function createselect(languages) {
    const select = document.getElementById('selectedLanguge');
    ...
    fetch(`${HOST}/api/routes/${route.id}/guides?api_key=${API_KEY}`)
        ...
        .then(response => {
            ...
            for (let item of response) {
                ...
                guidePhoto.src = 'guide.png';
                guidePhoto.classList = 'imgwdt';
...
function xmlRoutesFromApi() {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', `${HOST}/api/routes?api_key=${API_KEY}`, true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            console.error('Error XMLHttpRequest route data:', xhr.statusText);
            if (console.error('Error XMLHttpRequest route data:', 
                xhr.statusText)) {
                alert('Error XMLHttpRequest route data:', xhr.statusText);
            }    
        }
    };
    xhr.send();
...
//создание страниц для пагинации
...
//обновление пагинации после поиска роутов
...
//отслеживаем нажатия кнопки выбрать в гидах и если нажата, то возвращаем id
